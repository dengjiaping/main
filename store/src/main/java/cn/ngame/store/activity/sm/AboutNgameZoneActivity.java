...
public class AboutNgameZoneActivity extends BaseFgActivity {
    ...
    private static final String TAG = AboutNgameZoneActivity.class.getSimpleName();
    ...
    public void onUpdateClick(View view) {
        ...
        if (isDownloading) {
            if (isRunningBackground) {
                Toast.makeText(AboutNgameZoneActivity.this, "正在后台为您更新！", Toast.LENGTH_SHORT).show();
            } else {
                showProgressDialog();
            }
            return;
            ...
            public void onResponse(JsonResult<VersionInfo> result) {
                ...
                if (versionInfo != null) {
                    ...
                    if (localVersion < versionInfo.versionCode) {
                        showUpdateDialog();
                        CommonUtil.verifyStoragePermissions(AboutNgameZoneActivity.this); //申请读写SD卡权限
                    } else {
                            ...
                            if (isRunningBackground) {
                                ...
                                if (process >= 100) {
                                    ...
                                    notification = new Notification.Builder(context)
                                            .setSmallIcon(R.mipmap.ic_launcher)
                                            .setContent(remoteViews)
                                            .setContentIntent(PendingIntent.getActivity(AboutNgameZoneActivity.this, 0, intent,
                                                    0))
                                            .build();
